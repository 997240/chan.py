...
class BiList:
    ...
    def update_peak(self, klc: KLine, for_virtual=False):
    ...
    def try_update_end(self, klc: KLine, for_virtual=False) -> bool:
        def check_top(klc: KLine, for_virtual):
            if for_virtual:
                return klc.dir == KLINE_DIR.UP
            else:
                return klc.fx == FX_TYPE.TOP

        def check_bottom(klc: KLine, for_virtual):
            if for_virtual:
