...
from common.enums import BI_DIR, FX_TYPE, KL_TYPE, KLINE_DIR, TREND_TYPE
from common.chan_exception import ChanException, ErrCode
from common.ctime import CTime
from math_util.demark import T_DEMARK_INDEX, DemarkEngine

from .plot_meta import Bi_meta, ChanPlotMeta, ZS_meta
...
def cal_y_range(meta: ChanPlotMeta, ax):
...
def cal_x_limit(meta: ChanPlotMeta, x_range):
    X_LEN = meta.klu_len
...
def GetPlotMeta(chan: Chan, figure_config) -> List[ChanPlotMeta]:
    plot_metas = [ChanPlotMeta(chan[kl_type]) for kl_type in chan.lv_list]
...
class PlotDriver:
    def __init__(self, chan: Chan, plot_config: Union[str, dict, list] = '', plot_para=None):
    ...
    def GetRealXrange(self, figure_config, meta: ChanPlotMeta):
    ...
    def DrawElement(self, plot_config: Dict[str, bool], meta: ChanPlotMeta, ax: Axes, lv, plot_para, ax_macd: Optional[Axes], x_limits):
    ...
    def draw_klu(self, meta: ChanPlotMeta, ax: Axes, width=0.4, rugd=True, plot_mode="kl"):
    ...
    def draw_klc(self, meta: ChanPlotMeta, ax: Axes, width=0.4, plot_single_kl=True):
    ...
    def draw_eigen(self, meta: ChanPlotMeta, ax: Axes, color_top="r", color_bottom="b", aplha=0.5, only_peak=False):
        for eigenfx_meta in meta.eigenfx_lst:
            self.plot_single_eigen(eigenfx_meta, ax, color_top, color_bottom, aplha, only_peak)

    def draw_segeigen(self, meta: ChanPlotMeta, ax: Axes, color_top="r", color_bottom="b", aplha=0.5, only_peak=False):
    ...
    def draw_segzs(self, meta: ChanPlotMeta, ax: Axes, color='red', linewidth=10, sub_linewidth=4):
        linewidth = max(linewidth, 2)
        x_begin = ax.get_xlim()[0]
    ...
    def draw_macd(self, meta: ChanPlotMeta, ax: Axes, x_limits, width=0.4):
    ...
    def draw_mean(self, meta: ChanPlotMeta, ax: Axes):
    ...
    def draw_channel(self, meta: ChanPlotMeta, ax: Axes, T=None, top_color="r", bottom_color="b", linewidth=3, linestyle="solid"):
        ...
        if T is None:
            ...
            raise ChanException(f"plot channel of T={T} is not setted in ChanConfig.trend_metrics = {config_T_lst}", ErrCode.PLOT_ERR)
    ...
    def draw_boll(self, meta: ChanPlotMeta, ax: Axes, mid_color="black", up_color="blue", down_color="purple"):
        ...
        try:
            ...
            raise ChanException("you can't draw boll until you set boll_n in ChanConfig", ErrCode.PLOT_ERR) from e
    ...
    def draw_bs_point(self, meta: ChanPlotMeta, ax: Axes, buy_color='r', sell_color='g', fontsize=15, arrow_l=0.15, arrow_h=0.2, arrow_w=1):
    ...
    def draw_seg_bs_point(self, meta: ChanPlotMeta, ax: Axes, buy_color='r', sell_color='g', fontsize=18, arrow_l=0.2, arrow_h=0.25, arrow_w=1.2):
        self.bsp_common_draw(
    ...
    def draw_demark_begin_line(self, ax, begin_line_color, plot_begin_set: set, linestyle: str, demark_idx: T_DEMARK_INDEX):
        if begin_line_color is not None and demark_idx['series'].TDST_peak is not None and id(demark_idx['series']) not in plot_begin_set:
            ...
            ax.plot(
                [demark_idx['series'].kl_list[DemarkEngine.SETUP_BIAS].idx, end_idx],
    ...
    def draw_demark(
        ...
        for klu in meta.klu_iter():
            ...
            for demark_idx in klu.demark.get_countdown():
                box_bias = 0.5*text_height if text_height is not None and demark_idx['idx'] == DemarkEngine.MAX_COUNTDOWN else 0
                txt_instance = ax.text(
                    klu.idx,
                ...
                if demark_idx['idx'] == DemarkEngine.MAX_COUNTDOWN:
